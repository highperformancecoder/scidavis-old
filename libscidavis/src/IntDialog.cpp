        ...
        try {
            ...
            if (start < minx) {
                QMessageBox::warning((ApplicationWindow *)parent(), tr("Input error"),
                                     tr("Please give a number larger or equal to the minimum value "
                                        "of X, for the lower limit.\n If you do not know that "
                                        "value, type min in the box."));
                boxStart->clear();
                boxStart->setFocus();
                return;
            }
            if (start > maxx) {
                QMessageBox::warning((ApplicationWindow *)parent(), tr("Input error"),
                                     tr("Please give a number smaller or equal to the maximum "
                                        "value of X, for the lower limit.\n If you do not know "
                                        "that value, type max in the box."));
                boxStart->clear();
                boxStart->setFocus();
                return;
            }
        } catch (mu::ParserError &e) {
            QMessageBox::critical((ApplicationWindow *)parent(), tr("Start limit error"),
    ...
    if (end == "min") {
        ...
        try {
            ...
            if (stop > maxx) {
                ...
                /*QMessageBox::warning((ApplicationWindow *)parent(), tr("Input error"),
                        tr("Please give a number smaller or equal to the maximum value of X, for the
            ...
            QMessageBox::critical((ApplicationWindow *)parent(), tr("End limit error"),
                                  QStringFromString(e.GetMsg()));
            boxEnd->clear();
            boxEnd->setFocus();
            return;
...
void IntDialog::help()
{
    QMessageBox::about(
            ...
            tr("The integration of a curve consists of the following five steps:\n 1) Choose which "
               "curve you want to integrate\n 2) Set the order of the integration. The higher it "
               ...
               "integrates the curve with an iterative algorithm. The tolerance determines the "
               "termination criteria for the solver.\n Because, sometimes we ask for too much "
               "accuracy, the number of iterations makes sure that the solver will not work for "
               "ever.\n IMPORTANT \nThe limits must be within the range of x; If you do not know "
               "the maximum (minimum) value of x, type max (min) in the boxes."));
}
