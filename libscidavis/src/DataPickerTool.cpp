...
void DataPickerTool::setSelection(QwtPlotCurve *curve, int point_index)
{
    ...
    else
    {
        ...
        emit statusText(QString("%1[%2]: x=%3; y=%4")
			.arg(d_selected_curve->title().text())
			.arg(row + 1)
			.arg(t->column(xCol).textAt(row).c_str())
			.arg(t->column(yCol).textAt(row).c_str()));
    }
                  ...
                  if (d_graph && d_selected_curve)
                    {
                      ...
                      for (int i = start; i < start + n_curves; ++i)
                        if ((c=d_graph->curvePtr(i % n_curves))->dataSize() > 0) {
		...
		case Qt::Key_Plus:
                  if (d_graph)
                  {
                    if (d_selected_curve) {
                      ...
                      setSelection(d_graph->curvePtr(0), 0);
                  }
                  return true;
		...
		case Qt::Key_Minus:
                  if (d_graph)
                  {
                    if (d_selected_curve) {
                      ...
                      setSelection(d_graph->curvePtr(d_graph->curves()-1), 0);
                  }
                  return true;
