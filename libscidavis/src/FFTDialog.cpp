...
void FFTDialog::activateCurve(const QString& curveName)
{
	...
	else if (d_table)
	{
	    int col = d_table->colIndex(curveName);
            double x0 = stod(d_table->column(col).textAt(0));
            double x1 = stod(d_table->column(col).textAt(1));
...
void FFTDialog::setTable(Table *t)
{
	...
	if (xcol >= 0)
	{
		...
		double x0 = stod(t->column(xcol).textAt(0));
		double x1 = stod(t->column(xcol).textAt(1));
		boxSampling->setText(QString::number(x1 - x0));
	}
