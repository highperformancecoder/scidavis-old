...
public:
	...
	virtual QString name() const { return d_future_matrix->name();} 
	//! Set the window name
...
public slots:
	...
	double xStart() const{return d_future_matrix->xStart();};
	//! Return the X value corresponding to the last column
	double xEnd() const{return d_future_matrix->xEnd();};
	//! Return the Y value corresponding to row 1
	double yStart() const{return d_future_matrix->yStart();};
	//! Return the Y value corresponding to the last row
	double yEnd() const{return d_future_matrix->yEnd();};

	//! Returns the bounding rect of the matrix coordinates
  	QwtDoubleRect boundingRect() const {return QwtDoubleRect(xStart(), yStart(), xEnd()-xStart(), yEnd()-yStart()).normalized();};
