					...
					if (c && c->rtti() == QwtPlotItem::Rtti_PlotCurve)
					{
						if (d_file_version < 90)
							...
							c->setAxis
                                                        ...
                                                        c->setAxes
#endif
                                 (curve[curve.count()-2].toInt(), curve[curve.count()-1].toInt());
						{
#if QWT_VERSION<0x60000
							c->setAxis
#else
                                                        c->setAxes
#endif
							(curve[curve.count()-5].toInt(), curve[curve.count()-4].toInt());
				...
				if (c)
				{
					if(current_index+1 < curve.size())
					     ...
					     c->setAxis
                                             ...
                                             c->setAxes
#endif
						(curve[current_index].toInt(), curve[current_index+1].toInt());
