...
// shim class to give QwtScaleDraw copyiability
class CopyiableScaleDraw: public QwtScaleDraw
{
public:
  CopyiableScaleDraw() {}
#ifdef __GNUC__
...
#endif
  CopyiableScaleDraw(const CopyiableScaleDraw& x)  {operator=(x);}
  CopyiableScaleDraw(const QwtScaleDraw& x)  {operator=(x);}
  CopyiableScaleDraw& operator=(const CopyiableScaleDraw& x)
  {return operator=(static_cast<const QwtScaleDraw&>(x));}
  CopyiableScaleDraw& operator=(const QwtScaleDraw& x)
  {
    if (this!=&x)
      {
        // based on qwt 6.1 source code - hopefully the implementation
        // doesn't change
        setLength(x.length());
        setAlignment(x.alignment());
        setLabelAlignment(x.labelAlignment());
        setLabelRotation(x.labelRotation());
        move(x.pos());
      }
    return *this;
  }
...
class ScaleDraw: public CopyiableScaleDraw
{
public:
...
private:
  QwtTextScaleDraw(const ScaleDraw &other);
...
private:
  TimeScaleDraw(const ScaleDraw &other);
  QTime t_origin;
  QString t_format;
};
