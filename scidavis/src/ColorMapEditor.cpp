...
void ColorMapEditor::updateColorMap()
{
  ...
  color_map.reset(new QwtLinearColorMap(c_min, c_max));
  for (int i = 1; i < rows-1; i++)
    {
      double val = (table->item(i, 0)->text().toDouble() - min_val)/
        (max_val-min_val);
      color_map->addColorStop (val, QColor(table->item(i, 1)->text()));
    }

  setScaledColors(scaleColorsBox->isChecked());
}

void ColorMapEditor::setColorMap(const std::tr1::shared_ptr<QwtLinearColorMap>& map)
{
...
for (int i = 0; i < rows; i++)
	{
	double val = min_val + colors[i] * (max_val-min_val);
...
double val = 0.5*(table->item(row, 0)->text().toDouble() + table->item(row - 1, 0)->text().toDouble());
 double mapped_val = (val - min_val)/(max_val-min_val);
