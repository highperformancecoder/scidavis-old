...
void PlotDialog::setActiveCurve(CurveTreeItem *item)
{
    ...
    if (i->rtti() == QwtPlotItem::Rtti_PlotSpectrogram)
    {
        ...
        QwtDoubleInterval range=dynamic_cast<MatrixData&>(*sp->data()).range();
        colorMapEditor->setRange(range.minValue(), range.maxValue());
#if QWT_VERSION<0x60000
...
bool PlotDialog::acceptParams()
{
	...
	else if (privateTabWidget->currentWidget()==symbolPage)
	{
		if (boxSymbolStyle->selectedSymbol() == QwtSymbol::NoSymbol) // no symbol => line plot
			...
			curve->setSymbol(s);
#else
			curve->setSymbol(new QwtSymbol(s));
#endif
		}
	}
...
bool PlotDialog::validInput()
{
	...
	if (start>=end)
	{
		QMessageBox::critical(this, tr("Input error"), tr("Please enter limits that satisfy: begin < end!"));
		histogramEndBox->setFocus();
		return false;
	}
	...
	if (stp <=0)
	{
		QMessageBox::critical(this, tr("Bin size input error"), tr("Please enter a positive bin size value!"));
		binSizeBox->setFocus();
		return false;
	}
