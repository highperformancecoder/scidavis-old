...
d_user_step = QMemArray<bool>(QwtPlot::axisCnt);
...
d_zoomer[0]->setRubberBandPen(Qt::black);
...
{
QValueList<int> mrkKeys=d_plot->markerKeys();
int n=mrkKeys.size();
if (n==0)
	return;
...
if (selectedMarker>=0)
	...
	if (d_lines.contains(selectedMarker)>0)
		...
		{
		int index=d_texts.find(selectedMarker,0);
		...
		{
		int index=d_images.find(selectedMarker,0);
...
if (legendMarkerID >= 0)
	{
	int index = d_texts.find(legendMarkerID, 0);
	int texts = d_texts.size();
	for (int i=index; i<texts; i++)
		d_texts[i]=d_texts[i+1];
	d_texts.resize(--texts);
...
for (int i=0; i < QwtPlot::axisCnt; i++)
	...
	int prec;
	char f;
	d_plot->axisLabelFormat(i, f, prec);

	const QwtScaleDiv *scDiv=d_plot->axisScaleDiv(i);
	QwtValueList lst = scDiv->ticks (QwtScaleDiv::MajorTick);

	s += QString::number(QMIN(scDiv->lBound(), scDiv->hBound()), f, prec)+"\t";
	s += QString::number(QMAX(scDiv->lBound(), scDiv->hBound()), f, prec)+"\t";

	double step = 0.0;
	if (d_user_step[i])
		step = fabs(lst[1]-lst[0]);

	s += QString::number(step, f, prec)+"\t";
...
int imagesOnPlot=d_images.size();
d_images.resize(++imagesOnPlot);
d_images[imagesOnPlot-1]=mrk2ID;
}
...
void Graph::cutTitle()
{
QApplication::clipboard()->setData(new QTextDrag(d_plot->title().text(), d_plot->titleLabel(), 0));
removeTitle();
}
