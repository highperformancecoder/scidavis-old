...
#else
	s += "CanvasBackground\t" + d_plot->canvasBackground().color().name()+"\t";
...
#else
	QRect canvas_rect = d_plot->plotLayout()->canvasRect().toRect();
#endif
...
#else
		xCol->setValueAt(i, curve->sample(i).x());
		yCol->setValueAt(i, curve->sample(i).y());
#endif
	}
...
void Graph::plotBoxDiagram(Table *w, const QStringList& names, int startRow, int endRow)
{
	...
	for (int j = 0; j <(int)names.count(); j++){
        ...
        c->setData(double(j+1), QwtArray<double>(), 0);
        c->loadData();
...
void Graph::openBoxDiagram(Table *w, const QStringList& l, int fileVersion)
{
	...
	c->setData(l[1].toDouble(), QwtArray<double>(), 0);
	c->setData(l[1].toDouble(), QwtArray<double>(), 0);
	c->loadData();
...
void Graph::plotSpectrogram(Matrix *m, CurveType type)
{
	if (type != GrayMap && type != ColorMap && type != ContourMap)
  	...
  	rightAxis->setColorBarEnabled(type != ContourMap);
        // raster data should be a MatrixData - fail hard if not
        QwtDoubleInterval range=dynamic_cast<MatrixData&>(*d_spectrogram->data()).range();
        //TODO - I'm really, really dubious about this code. The API
        //doesn't mentioned whether ownership of the colourmap is
        //passed here. ColourMaps are abstract classes, so I can't
        //exactly create a copy. We should study the Qwt source code
        //to figure what in blazes is supposed to be going on
        //here. Farrrk!!
  	...
  	d_plot->setAxisScale(QwtPlot::yLeft, m->yStart(), m->yEnd());

        {
          QwtDoubleInterval range=
            dynamic_cast<MatrixData&>(*d_spectrogram->data()).range();
          d_plot->setAxisScale(QwtPlot::yRight,range.minValue(),range.maxValue());
        ...
        const QwtPlotPrintFilter &pfilter)
{
	...
	int bw = d_plot->lineWidth();
        // TODO - check this doesn't bugger printing.
    ...
    int bw = d_plot->canvas()->lineWidth();
#if QWT_VERSION<0x60000
    QRect canvasRect = d_plot->plotLayout()->canvasRect().adjusted(bw, bw, -bw, -bw);
#else
    QRect canvasRect = d_plot->plotLayout()->canvasRect().adjusted(bw, bw, -bw, -bw).toRect();
#endif
...
#else
            QRect scaleRect = d_plot->plotLayout()->scaleRect(axisId).toRect();
#endif
