...
#else
          double xx=sample(i).x(), yy=sample(i).y();
#endif
		const int xi = xMap.transform(xx + d_xOffset);
		const int yi = yMap.transform(yy + d_yOffset);
		if (type == Vertical){
			int y_plus = yMap.transform(yy+err[i]);
			int y_minus = yMap.transform(yy-err[i]);
			bool y_minus_is_finite = true;

			if (yMap.transformation()->type() == QwtScaleTransformation::Log10 && err[i] >= yy) {
			...
			int x_plus = xMap.transform(xx+err[i]);
			int x_minus = xMap.transform(xx-err[i]);
			bool x_minus_is_finite = true;

			if (xMap.transformation()->type() == QwtScaleTransformation::Log10 && err[i] >= xx) {
